...
//C# TO JAVA CONVERTER TODO TASK: There is no preprocessor in Java:
	...
	public final void calSample(Graphics g)
	{
		...
		Line line = panel.newLine(); // title
		line.newSpan("テスト", panel.titleFont, panel.titleBrush, g.MeasureString("テスト", panel.titleFont));
		line.newSpan("ケース", panel.titleFont, panel.titleBrush, g.MeasureString("ケース", panel.titleFont));

		line = panel.newLine(); // graw line

		line = panel.newLine();
		line.newSpan("3", g.MeasureString("3", panel.font));
		line.newSpan("現在", panel.font, panel.brush, g.MeasureString("現在", panel.font));
		line.newSpan("の", panel.font, panel.brush, g.MeasureString("の", panel.font));
		line.newSpan("テストケース", panel.font, panel.mainItemBrush, g.MeasureString("テストケース", panel.font));
		line.newSpan(null, "⇒", panel.font, panel.brush, g.MeasureString("⇒", panel.font));

		line = panel.newLine();
		line.newSpan("2", g.MeasureString("3", panel.font));
		line.newSpan("テストケース", panel.font, panel.mainItemBrush, g.MeasureString("テストケース", panel.font));
		line.newSpan("を", panel.font, panel.brush, g.MeasureString("を", panel.font));
		line.newSpan("作る", panel.font, panel.brush, g.MeasureString("作る", panel.font));
		line.newSpan("ため", panel.font, panel.brush, g.MeasureString("ため", panel.font));
		line.newSpan("に", panel.font, panel.brush, g.MeasureString("に", panel.font));
		line.newSpan("は", panel.font, panel.brush, g.MeasureString("は", panel.font));
		line.newSpan(null, "⇒", panel.font, panel.brush, g.MeasureString("⇒", panel.font));
